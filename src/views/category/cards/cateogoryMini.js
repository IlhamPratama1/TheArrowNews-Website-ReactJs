...
export function MiniCategoryCard({ title, posts }) {
    return(
        <div className="bg-blue w-full p-2 pl-4 rounded-sm">
...
export function MiniCategoryCard2({ title }) {
    return(
        <div className="border-blue w-full p-2 pl-4 rounded-sm border">
...
export function AddCategoryCard() {
    return(
        <Link to='/admin/category' className="bg-gray w-full p-2 pl-4 rounded-sm flex items-center space-x-10 text-black">
            <h1 className="font-oswald font-bold text-2xl uppercase">Add new</h1>
            <img className="w-5" src="/static/icons/add.svg" alt="add" />
        </Link>
    );
}